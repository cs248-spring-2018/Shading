...
void Application::resize(size_t w, size_t h) {
  ...
  if (mode != MODEL_MODE || mode != SHADER_MODE) {
...
string Application::name() { return "Render"; }
...
void Application::executeFileOp(int codepoint) {
  ...
  filename << "Cardinal3D_buffer" << index << ".dae";
...
void CS248::Application::writeSkeleton(const char *filename,
...
void CS248::Application::loadSkeleton(const char *filename,
...
void Application::switch_modes(unsigned int key) {
  // TODO: re-enable this after fixing all key conflicts to support various modes
  /*
  switch (key) {
    case 'a':
    case 'A':
